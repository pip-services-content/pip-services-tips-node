# Configuration Guide <br/> Attachments Microservice

Attachments microservice configuration structure follows the 
...
- path: string - file path where AttachmentsV1 objects are stored. The object are written into the file in JSON format.
- options: object - Misc configuration options
  - max_page_size: number - Maximum number of items per page (default: 100)
...
Example:
```yaml
- descriptor: "pip-services-attachments:service:seneca:default:1.0"
  connection:
    protocol: "http"
    host: "0.0.0.0"
    port: 3000
